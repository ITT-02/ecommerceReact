  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Typography variant="subtitle2" color="text.secondary" fontWeight={600}>Monto:</Typography>
...
const HistorialItem = ({ hist, theme }) => {
    ...
    <Box sx={{ display: 'flex', gap: 1.5, mb: 2, alignItems: 'flex-start' }}>
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Typography variant="body2" sx={{ lineHeight: 1.4 }}>
          <Box component="span" fontWeight={700}>{getDisplayName(hist.cambiado_por)}</Box>
          {' '}cambió a{' '}
          <Box component="span" fontWeight={700} sx={{ textTransform: 'uppercase', color: palette.main }}>
            {hist.estado_nuevo}
          </Box>
        </Typography>
  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <>
          {/* Margen negativo para que el banner toque los bordes del área de contenido */}
          <Box sx={{ mx: { xs: -2.25, sm: -3 }, mt: { xs: -2.75, sm: -3 }, mb: 3 }}>
            <StatusHeroBanner estado={detail.estado} monto={detail.monto} />
          </Box>
          ...
          >
            ...
            <SectionCard title="Evidencia" icon={<VerifiedUserIcon color="primary" />}>
              <Box
                sx={{
              ...
              >
                {detail.url_comprobante ? (
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    <Typography color="text.disabled" variant="body2" fontWeight={600}>Sin comprobante</Typography>
                  </Box>
                )}
              </Box>
            </SectionCard>

            <SectionCard title="Cliente" icon={<PersonOutlineOutlinedIcon color="primary" />}>
              ...
              <DataRow label="Correo" value={detail.correo_cliente} />
            </SectionCard>

            <SectionCard title="Historial" icon={<HistoryIcon color="primary" />}>
              {detail.historial_estados?.length > 0 ? (
                <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, overflowY: 'auto' }}>
                  {detail.historial_estados.slice(0, 3).map((hist) => (
                    <HistorialItem key={hist.id} hist={hist} theme={theme} />
                  ))}
                </Box>
              ) : (
                <Typography variant="body2" color="text.disabled" align="center" sx={{ py: 3, flex: 1 }}>
