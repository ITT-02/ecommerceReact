...
const getAttributesText = (attributes = []) => {
  ...
  return attributes.map((i) => `${i.atributo_nombre || 'Atributo'}: ${i.valor}`).join(' · ');
};
...
const getOrderMessage = (quote = {}) => {
  ...
  if (quote.estado_pago === 'pagado')
    return `La cotización generó el pedido ${quote.numero_pedido || quote.pedido_id}. Pago confirmado. Pedido: ${orderText}.`;
  ...
  return `La cotización generó el pedido ${quote.numero_pedido || quote.pedido_id}. Pedido: ${orderText}. Pago: ${paymentText}.`;
};
      ...
      <Stack spacing={2.5}>
        ...
        {quote.canal_venta === 'manual' && (
          <Alert severity="info">
            <Typography variant="subtitle2" fontWeight={900}>
              Cotización asistida por vendedor
            </Typography>
            <Typography variant="body2">
              La respuesta se gestiona desde el panel administrativo.
            </Typography>
          </Alert>
        )}
        ...
        {orderMessage && (
          <Alert severity={quote.estado_pago === 'pagado' ? 'success' : 'warning'}>
            <Typography variant="subtitle2" fontWeight={900}>Seguimiento del pedido</Typography>
        ...
        {!quote.pedido_id && ['respondida', 'vencida', 'cancelada', 'rechazada'].includes(quote.estado) && (
          <Alert severity="info">Esta cotización todavía no generó pedido.</Alert>
        )}
            ...
            return (
              ...
              >
                <Stack spacing={1}>
                  ...
                  {item.notas_cliente && (
                    <Alert severity="info">Detalle del producto: {item.notas_cliente}</Alert>
                  )}
