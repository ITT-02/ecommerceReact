...
export const ProductGallery = ({
  ...
  const items = useMemo(() => {
    ...
    return source
      ...
      .filter(Boolean);
  }, [media, fallbackImage, productName]);

  // Firma estable para detectar cuando la galería cambió por selección de variante.
  const itemsSignature = useMemo(
    () => items.map((item) => `${item.id ?? 'sin-id'}:${item.type}:${item.src}`).join('|'),
    [items],
  );
  ...
  // Guarda qué video flotante fue cerrado para no volver a mostrarlo en el mismo set de medios.
  const [dismissedFloatingVideoSignature, setDismissedFloatingVideoSignature] = useState('');
  ...
  // Primer video disponible: se muestra como mini reproductor automático
  // sobre la sección de imágenes.
  const firstFloatingVideo = useMemo(
    () => items.find((item) => item.type === 'video' && !brokenSrcs.has(item.src)) ?? null,
    [items, brokenSrcs],
  );

  const firstFloatingVideoSignature = firstFloatingVideo
    ? `${firstFloatingVideo.id ?? 'sin-id'}:${firstFloatingVideo.src}`
    : '';

  const showMiniFloatingVideo = Boolean(
    firstFloatingVideo &&
    firstFloatingVideoSignature &&
    dismissedFloatingVideoSignature !== firstFloatingVideoSignature,
  );

  // Cuando cambia la lista de medios (por ejemplo, al seleccionar una variante),
  // la galería vuelve al primer elemento para que la imagen principal sea la correcta.
  useEffect(() => {
    setActiveIndex(0);
    setAnimKey((k) => k + 1);
  }, [itemsSignature]);
  ...
  useEffect(() => {
    if (!isVertical || !viewerBoxRef.current) return undefined;
  ...
  const closeMiniFloatingVideo = useCallback(() => {
    if (!firstFloatingVideoSignature) return;
    setDismissedFloatingVideoSignature(firstFloatingVideoSignature);
  }, [firstFloatingVideoSignature]);
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* ── Video en visor: solo previsualización silenciosa ─ */}
            {isVideo && !srcBroken && (
              <Box
                key       = {`vid-${animKey}`}
                ref       = {videoRef}
            ...
            {isVideo && !srcBroken && (
              ...
              >
                <PlayCircleRoundedIcon
                  sx={{ color: (t) => t.palette.common.white, fontSize: 13 }}
                />
                <Typography
                  variant = "caption"
                  sx      = {{
            ...
            {/* ── Mini video automático: flota sobre la imagen principal ─ */}
            {showMiniFloatingVideo && (
              <MiniFloatingVideo
                video={firstFloatingVideo}
                title={productName}
                onClose={closeMiniFloatingVideo}
              />
            )}
          </Box>
        </Box>
...
// ─── MiniFloatingVideo ─────────────────────────────────────────────
const MiniFloatingVideo = ({ video, title, onClose }) => {
  ...
  return (
    <Box
      sx={(theme) => {
        ...
        return {
          ...
          bottom: { xs: 10, sm: 14 },
          zIndex: 5,
          /* Tamaño pequeño, pero suficiente para que los controles nativos
           * del navegador se puedan usar sin abrir otro modal. */
          width: { xs: 184, sm: 220, md: 240 },
          aspectRatio: '16/10',
          overflow: 'hidden',
          borderRadius: `${radius.lg}px`,
          border: '1px solid',
          borderColor: semantic.storeNavigation?.border ?? semantic.border,
          bgcolor: semantic.paper,
          boxShadow: theme.palette.custom.shadows.lg,
          transformOrigin: 'bottom right',
          animation: `miniVideoIn ${motion.durationSlow} ${motion.easeOut}`,
          '@keyframes miniVideoIn': {
    ...
    >
      {/*
       * Video pequeño visible desde que el cliente ingresa al detalle.
       * Usa controles nativos para que el cliente pueda: reproducir, pausar,
       * ajustar volumen y entrar a pantalla completa desde el mismo video,
       * sin abrir un segundo reproductor.
       */}
      <Box
        component="video"
        key={video.src}
        ...
        playsInline
        preload="metadata"
        aria-label={`Video del producto ${title || ''}`}
        sx={(theme) => ({
          width: '100%',
          height: '100%',
          display: 'block',
      ...
      {/* El cliente puede cerrar el mini video si le resulta molesto. */}
      <IconButton
        type="button"
        size="small"
        onClick={onClose}
        aria-label="Cerrar mini video"
        sx={(theme) => ({
          position: 'absolute',
          top: 5,
          right: 5,
...
const MediaBadge = ({ bottom, right, top, left, children }) => (
  ...
  >
    <Typography
      variant = "caption"
      sx      = {{ color: (t) => t.palette.common.white, fontWeight: 700, lineHeight: 1.5 }}
    >
      {children}
    </Typography>
  </Box>
        ...
        >
          <PlayCircleRoundedIcon
            sx={{
              color : (t) => t.palette.common.white,
              fontSize: 26,
              filter: (t) => `drop-shadow(0 1px 4px ${alpha(t.palette.common.black, 0.60)})`,
            }}
          />
        </Box>
      )}
